...
function render_admin_page(string $body, string $title='Admin'): void { header('Content-Type: text/html; charset=utf-8'); echo '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'.html_escape($title).' - Airside LogTen</title><style>body{margin:0;background:#f5f7fb;color:#111827;font-family:Inter,system-ui,-apple-system,Segoe UI,Arial,sans-serif}.wrap{max-width:1050px;margin:28px auto;padding:0 18px}.top{display:flex;justify-content:space-between;gap:14px;align-items:center;margin-bottom:18px}.brand h1{margin:0;font-size:30px;letter-spacing:-.04em}.brand p,.muted,.help{color:#6b7280;font-size:13px;line-height:1.45}.panel{background:white;border:1px solid #e5e7eb;border-radius:22px;box-shadow:0 20px 60px rgba(15,23,42,.10);overflow:hidden}.section{margin-top:18px}.head{padding:20px 22px 12px;border-bottom:1px solid #e5e7eb}.head h2{margin:0;font-size:20px}.head p{margin:6px 0 0;color:#6b7280}.body{padding:18px 22px 22px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}@media(max-width:760px){.grid{grid-template-columns:1fr}}label{display:block;font-weight:800;font-size:13px;margin-bottom:6px}input,textarea{width:100%;border:1px solid #d1d5db;border-radius:12px;padding:11px 12px;font:inherit;background:white}textarea{min-height:120px}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:14px}.btn,button{display:inline-block;border:0;border-radius:13px;padding:11px 14px;font-weight:850;text-decoration:none;cursor:pointer}.primary{background:#2563eb;color:white}.secondary{background:#eef2ff;color:#1f2937}.ghost{background:#f3f4f6;color:#374151}.danger{background:#fee2e2;color:#b91c1c}.alert{padding:12px 14px;border-radius:14px;margin-bottom:14px;font-weight:650}.ok{background:#ecfdf5;color:#15803d;border:1px solid #bbf7d0}.warn{background:#fffbeb;color:#b45309;border:1px solid #fde68a}.err{background:#fef2f2;color:#b91c1c;border:1px solid #fecaca}.stat{border:1px solid #e5e7eb;border-radius:16px;padding:14px;background:#f9fafb}.stat b{display:block;font-size:18px}.stat span{display:block;color:#6b7280;font-size:12px;margin-top:4px}code{background:#f3f4f6;padding:2px 5px;border-radius:6px}</style></head><body><main class="wrap"><div class="top"><div class="brand"><h1>Airside ⟶ LogTen Admin</h1><p>'.APP_VERSION.' · Wasmer backend settings</p></div><div><a class="btn ghost" href="/">Open app</a></div></div>'.$body.'</main></body></html>'; }
...
function handle_admin(): void {
    ...
    if(admin_require_post()){
        ...
        elseif(admin_is_logged_in($db)){
            ...
            elseif($action==='restore_backup'){ $json=''; if(!empty($_FILES['backup_file']['tmp_name']) && is_uploaded_file($_FILES['backup_file']['tmp_name'])) $json=(string)file_get_contents($_FILES['backup_file']['tmp_name']); else $json=(string)($_POST['backup_json']??''); $p=json_decode($json,true); if(!is_array($p)){ $message='Backup restore failed: invalid JSON.'; $class='err'; } else { $n=restore_backup($db,$p,!empty($_POST['replace_cache'])); $message='Backup restored: '.$n['settings'].' setting(s), '.$n['cache'].' cache row(s).'; } }
